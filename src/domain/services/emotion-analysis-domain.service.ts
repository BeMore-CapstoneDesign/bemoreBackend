...
import { IEmotionAnalysisService, EmotionAnalysisInput, EmotionAnalysisResult, VadResult } from '../../common/interfaces/services.interface';

@Injectable()
export class EmotionAnalysisDomainService implements IEmotionAnalysisService {
  private readonly analysisServerUrl: string;

  constructor(private configService: ConfigService) {
    const url = this.configService.get<string>('ANALYSIS_SERVER_URL');
    if (!url) {
      throw new Error('ANALYSIS_SERVER_URL is not configured');
    }
    this.analysisServerUrl = url;
  }

  async analyzeEmotion(data: EmotionAnalysisInput): Promise<EmotionAnalysisResult> {
    const { image, audio, text } = data;
